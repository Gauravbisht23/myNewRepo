...
function _fnSetCalendar(e, sInputID, sCustom)
{
	...
	if(sCustom != undefined || sCustom != '')
	{
		...
		if(sCustom.indexOf('view') != -1)
		{
			var sView = _fnGetValue(sCustom, 'view');
			if(sView == 'mm' || sView == 'yy')
			{
				sView = sView;
			}
		}
...
function _fnCreateCalendar(iIndex)
{
	...
	if(oCalendarArray['View'] == 'mm')
		_fnShowAllMonths(iIndex);
	else if(oCalendarArray['View'] == 'yy')
		_fnShowYears(iIndex);
}
...
function _fnGetSelectedYear(iIndex, iYear)
{
	...
	if(oCalendarArray['View'] == 'yy')
		_fnAssignDate(iIndex, '1');
	else
		_fnCreateCalendar(iIndex);
	oCalendarArray			= null;
}
